...
class _TimeListViewState extends State<TimeListView> {
    ...
    setState(() {
      scheduleData[formattedDate] = List.generate(timeList.length, (index) {
        ...
        final resultEventsForTime = resultEvents.where((resultEvent) {
          try {
            final retSttTime = DateTime.parse(resultEvent.eventResultSttTime);
            final retEndTime = DateTime.parse(resultEvent.eventResultEndTime);
        ...
        final eventForTime = eventsForTime.isNotEmpty
            ? eventsForTime.first
            : null;

        final resultEventForTime = resultEventsForTime.isNotEmpty
            ? resultEventsForTime.first
            : null;
  ...
  void _copyEventToResult(int index) async {
    ...
    final eventSttTime = DateTime(
        eventDate.year, eventDate.month, eventDate.day, startTime + index);
    final eventEndTime = eventSttTime.add(Duration(hours: 1));
    ...
    if (eventSnapshot.docs.isNotEmpty) {
      ...
      if (event.eventSttTime != null && event.eventEndTime != null) {
        final eventDuration = event.eventEndTime!.difference(eventSttTime);

        if (eventDuration == Duration(hours: 1)) {
          final eventResult = EventResultModel(
            ...
            eventResultDate: eventDate.toIso8601String(),
            eventResultSttTime: event.eventSttTime!.toIso8601String(),
            eventResultEndTime: event.eventEndTime!.toIso8601String(), eventId: '', completeYn: '',
          );

          final docRef = await FirebaseFirestore.instance
              .collection('result_events')
              .add(eventResult.toMap());
          ...
          setState(() {
            scheduleData[formattedDate]?[index]['actual'] = eventTitle;
          });
        }
      }
    }
                ...
                DataRow(cells: [
                  DataCell(
                    Row(children: [
                      IconButton(
                        icon: Icon(
                          selectedStates[index] ? Icons.check_box : Icons.check_box_outline_blank,
                        ),
                        onPressed: () {
                    ...
                    onTap: () async {
                      ...
                      if (eventTitle == null || eventTitle.isEmpty) {
                        ...
                        final snapshot = await FirebaseFirestore.instance
                            .collection('events')
                            .where(
                            'eventDate', isEqualTo: eventDate.toIso8601String())
                            .where('eventTitle', isEqualTo: eventTitle)
                            .get();
                  ...
                  DataCell(
                    ...
                    onTap: () async {
                      ...
                      final isPlanEvent = scheduleData[formattedDate]?[index]['plan'] ==
                          eventTitle;
                      ...
                      if (eventTitle == null || eventTitle.isEmpty) {
                        ...
                        final snapshot = await FirebaseFirestore.instance
                            ...
                            .where('eventRetTitle', isEqualTo: eventTitle)
                            .get();

                        if (snapshot.docs.isNotEmpty) {
                          final eventData = snapshot.docs.first.data();
                          final eventResult = EventResultModel.fromMap(
                              eventData);
